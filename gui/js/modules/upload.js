  ...
  if (isFolder) {
    ...
    li.innerHTML = `
            <a href="javascript:void(0)" data-folder-path="${fullPath.replace(/"/g, "&quot;")}">
                ${cfg.icons.caret} ${cfg.icons.folder} <span>${name}</span> ${renderLockIcon("")}
            </a>`;
  } else {
...
export async function handleRemoteDownload() {
  ...
  document.getElementById("up-current-filename").innerText =
  ...
  try {
    ...
    if (data.success) {
      showConfirmDialog(
        "Success",
        "File imported from URL successfully.",
        "success",
      );
...
export async function handleFileUpload(files, destination) {
  ...
  for (let i = 0; i < files.length; i++) {
      ...
      try {
        await new Promise((resolve, reject) => {
          ...
          xhr.upload.onprogress = (e) => {
            ...
            if (timeDiff > 0.5) {
              const speedMBps = (
                (totalUploaded - lastLoaded) /
                timeDiff /
                (1024 * 1024)
              ).toFixed(1);
              if (fName)
                fName.innerText = `[${i + 1}/${files.length}] Uploading: ${file.name} (${speedMBps} MB/s)`;
              startTime = now;
    ...
    if (!window.cancelUpload) {
      ...
      if (fName)
        fName.innerText = `[${i + 1}/${files.length}] Checking integrity...`;
      ...
      try {
        const vRes = await fetch(window.location.href, {
          method: "POST",
          body: verifyData,
        });
...
export function initDropZone() {
  ...
  ["dragenter", "dragover"].forEach((ev) =>
    dropZone.addEventListener(
      ev,
      () => (dropZone.style.borderColor = "#3875d6"),
    ),
  );
...
export function initSidebarDragDrop() {
  ...
  sidebar.addEventListener("drop", (e) => {
    ...
    if (targetFolder) {
      destination =
        targetFolder.querySelector("a")?.getAttribute("data-folder-path") ??
        cfg.jailRoot;
    } else {
  ...
  document.addEventListener("click", (e) => {
    ...
    document
      ...
      .forEach((el) => el.classList.remove("is-selected-target", "active"));
