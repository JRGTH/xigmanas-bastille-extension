...
export async function executeDownloadRequest(type) {
  ...
  try {
    ...
    if (!data.success) {
      ...
      showConfirmDialog(
        "Error",
        data.error || "Unknown compression error from server",
        "error",
      );
      return;
    }
...
export function initPendingJobResume() {
  ...
  if (!pending) return;
  try {
    const { job_id, filename, jailName, csrfToken, parentDir } =
      JSON.parse(pending);
...
export async function showFileInfo(filePath) {
  ...
  try {
    ...
    if (currentFileData.success) {
      // Update Header
      document.getElementById("sidebar-filename").innerText =
    ...
    content.innerHTML =
      '<div class="modern-card" style="color:red">Connection error</div>';
  }
}
